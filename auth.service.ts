...
export class AuthService {
  ...
  changePassword(data) {
    return this.http.get(this.env.API_URL + 'changepassword/'+ data.username + '/' + data.password  + '/' + data.newpassword).pipe(
      tap((token: any) => {
        if (token.email) {
          const data = {'email': token.email, 'password': token.password};
          localStorage.setItem('currentUser', JSON.stringify(data));
          this.token = data;
          this.isLoggedIn = true;
          this.currentUserSubject.next(this.token);
  ...
  forgotPassword(data) {
    return this.http.get(this.env.API_URL + 'forgotpassword/'+ data.username + '/' + data.password).pipe(
      tap((token: any) => {
        if (token.email) {
          const data = {'email': token.email, 'password': token.password};
          localStorage.setItem('currentUser', JSON.stringify(data));
